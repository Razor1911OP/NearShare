...
function triggerFileDownload(upload, file) {
  ...
  a.click()
  // Remove after a short delay so the browser can start the download
...
async function downloadAllFiles(upload) {
  ...
  for (let i = 0; i < files.length; i++) {
    triggerFileDownload(upload, files[i])
...
function DropZone({ onFiles }) {
  ...
  const processEntries = useCallback(async (items) => {
    ...
    // Recursively read directory entries — handles >100 files per folder
    async function readEntry(entry, path = '') {
      if (entry.isFile) {
        ...
        // readEntries only returns ~100 entries at a time; loop until empty
        const readAll = () =>
          new Promise((res) => {
            const all = []
            const batch = () => {
  ...
  const handleDrop = useCallback(async (e) => {
    ...
    try {
      ...
      // no-op: global handler in App.jsx handles fallback
    }
    ...
    <motion.div
      ...
      animate={{
        ...
        borderColor:  draggingOver
          ? 'rgba(129,154,148,0.65)'
          : 'var(--surface-hi)',
        scale:        draggingOver ? 1.015 : 1,
        boxShadow:    draggingOver
          ? '0 0 0 4px rgba(129,154,148,0.10), inset 0 0 40px rgba(129,154,148,0.06)'
          : '0 0 0 0px rgba(129,154,148,0)',
      }}
      transition={{ duration: 0.18 }}
      style={{
        border:         '2px dashed var(--surface-hi)',
...
export default function DesktopView({ sendMessage, isMobile }) {
  ...
  const { uploadFiles, isUploading, uploadProgress, uploadStats } = useTransfer(sendMessage)
  ...
  const unreadCount = unreadChats?.size || 0
    ...
    <div className="ns-layout" style={{ gridTemplateRows: '60px 1fr' }}>
      ...
      <header className="ns-header">
        ...
        <div className="ns-logo">
          <img src="/icon-192.png" alt="" className="ns-logo-icon" aria-hidden="true" style={{ width: 36, height: 36, borderRadius: 10 }} />
          <span className="ns-wordmark">NearShare</span>
        </div>
      ...
      {/* ── Main content — two vertically stacked partitions ──────────────── */}
      <main style={{
        display:          'grid',
        ...
        {/* ── Partition 1: Devices + Send ─────────────────────────────────── */}
        <div className="ns-partition" style={{
          display:             'grid',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                <img
                  ...
                  style={{
                    ...
                    boxShadow: '0 2px 0 rgba(0,0,0,0.5), 0 8px 20px rgba(233,131,137,0.22)',
                  }}
                />
                <div style={{ minWidth: 0 }}>
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                      <div style={{
                        width: 36, height: 36, borderRadius: '50%',
                        background: isOnline
                          ? 'linear-gradient(145deg, rgba(26,35,32,0.9), rgba(17,23,21,0.9))'
                          : 'rgba(30,38,35,0.6)',
                        border: `2px solid ${isOnline ? 'rgba(141,186,164,0.45)' : 'var(--surface-hi)'}`,
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                <button
                  className={`ns-btn sm ${activeTab === 'files' ? 'primary' : 'ghost'}`}
                ...
                <button
                  className={`ns-btn sm ${activeTab === 'notes' ? 'primary' : 'ghost'}`}
              ...
              {activeTab === 'files' && selectedFiles.length > 0 && (
                <span className="ns-badge info">
                  {selectedFiles.length} staged · {formatBytes(totalBytes)}
                </span>
              )}
            </div>

            <div style={{ flex: 1, overflowY: 'auto', padding: activeTab === 'notes' ? 0 : '20px' }}>
              {activeTab === 'files' ? (
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                          ...
                          <button
                            ...
                            style={{ color: 'var(--bad)', borderColor: 'rgba(217,96,95,0.25)' }}
                          >
                            Clear all
                          </button>
                        </div>
                    ...
                    {selectedFiles.length > 0 && (
                      ...
                      >
                        <motion.button
                          ...
                          whileHover={isUploading ? {} : { scale: 1.02, boxShadow: '0 14px 36px rgba(129,154,148,0.40)' }}
        ...
        {/* ── Partition 2: Activity ───────────────────────────────────────── */}
        <div className="ns-partition">
            <div className="ns-partition-head">
              <h2 className="ns-display" style={{ margin: 0, fontSize: '0.95rem' }}>Activity</h2>
              {transfers.length > 0 && (
                <span className="ns-badge info">{transfers.length}</span>
              )}
            </div>
            ...
            <div style={{ flex: 1, minHeight: 0, padding: '14px 16px 16px' }}>
              <ActivityFeed
                ...
                liveCount={selectedFiles.length}
