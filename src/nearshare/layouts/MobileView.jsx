  ...
  return (
    <motion.div
      ...
      style={{
        ...
        borderLeft:     '3px solid var(--pewter-line)',
        boxShadow:      '0 2px 0 rgba(0,0,0,0.4), 0 8px 20px rgba(0,0,0,0.35)',
        touchAction:    'pan-y',
        cursor:         'grab',
        userSelect:     'none',
        overflow:       'hidden',
    ...
    >
      ...
      <div style={{
        ...
        background:     'linear-gradient(90deg, transparent, rgba(217,96,95,0.85))',
        borderRadius:   'var(--r)',
        pointerEvents:  'none',
        color:          '#fff',
        fontSize:       '1rem',
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        background:    'linear-gradient(180deg, var(--surface-2), var(--surface))',
        border:        '1px solid var(--border)',
        boxShadow:     '0 2px 0 rgba(0,0,0,0.38), 0 8px 18px rgba(0,0,0,0.3)',
      }}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 14px 0' }}>
        ...
        <motion.button
          ...
          style={{
            ...
            justifyContent: 'center',
            gap:            10,
            background:     'rgba(129,154,148,0.08)',
            border:         '2px dashed rgba(129,154,148,0.28)',
            borderRadius:   'var(--r)',
            padding:        '18px 14px',
            flexDirection:  'column',
            ...
            >
              ...
              <div style={{
                ...
                <button
                  ...
                  onClick={handleClear}
                  style={{ color: 'var(--bad)', borderColor: 'rgba(217,96,95,0.20)', minHeight: 'unset', padding: '4px 10px', fontSize: '0.76rem' }}
                >
                  Clear
                </button>
              </div>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 11, minWidth: 0 }}>
                  <div style={{
                    width: 40, height: 40, borderRadius: '50%',
                    background: online
                      ? 'linear-gradient(145deg, rgba(26,35,32,0.95), rgba(17,23,21,0.95))'
                      : 'rgba(30,38,35,0.6)',
                    border: `2px solid ${online ? 'rgba(141,186,164,0.45)' : 'var(--surface-hi)'}`,
...
// ─── Activity Tab ─────────────────────────────────────────────────────────────
function HistoryTab({ isUploading, uploadProgress, uploadStats, liveCount, liveBytes, liveTarget }) {
  return (
    <div style={{ flex: 1, minHeight: 0, padding: '14px 14px 6px' }}>
      <ActivityFeed
        compact
...
export default function MobileView({ sendMessage, isMobile }) {
  ...
  const liveBytes = (selectedFiles ?? []).reduce((sum, e) => sum + (e.file?.size ?? 0), 0)
  const liveTarget = state.selectedTargetId === 'host'
    ? 'Host'
    : (state.devices?.find(d => d.id === state.selectedTargetId)?.name ?? 'Device')
  ...
  return (
    <>
      ...
      <div className="ns-layout">
        ...
        {/* ── Header ──────────────────────────────────────────────────────── */}
        <header className="ns-header" style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}>
          {/* Logo */}
          <div className="ns-logo">
            ...
            />
            <span className="ns-wordmark" style={{ fontSize: '1rem' }}>NearShare</span>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {/* Settings */}
            <motion.button
              className="ns-btn pewter sm icon"
              onClick={() => setSettingsOpen(true)}
              whileTap={{ scale: 0.88 }}
              title="Settings"
              aria-label="Open settings"
              style={{ minHeight: 'unset', width: 38, height: 38, borderRadius: 11, padding: 0, fontSize: '1rem' }}
            >
              ⚙
            </motion.button>
          </div>
        ...
        <main style={{ overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column' }}>
          <AnimatePresence mode="wait" initial={false}>
            ...
            >
              ...
              {activeTab === 'notes' && <NotesPanel compact />}
        ...
        <nav style={{
          ...
          {TABS.map(tab => {
            ...
            if (tab.id === 'notes' && unreadChats?.size > 0) badge = unreadChats.size
                  ...
                  {isActive && (
                    <motion.div
                      ...
                      style={{
                        ...
                        height:      32,
                        borderRadius: '50%',
                        background:  'rgba(129,154,148,0.12)',
                        zIndex:      0,
                      }}
                    />
                  )}
      ...
      <SettingsPanel mobile open={settingsOpen} onClose={() => setSettingsOpen(false)} />
    </>
  )
}
