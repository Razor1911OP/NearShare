  ...
  while (attempt < retries) {
    ...
    try {
      const chunkRes = await fetch(
        `/api/upload/session/${encodeURIComponent(uploadId)}/chunk?fileIndex=${fileIndex}&chunkIndex=${chunkIndex}`,
        {
          method: 'PUT',
...
export default function useTransfer(sendMessage) {
  ...
  const uploadFiles = useCallback(
    async (opts = {}) => {
      ...
      const staged = state.selectedFiles
      ...
      try {
        const startRes = await fetch('/api/upload/session/start', {
          method: 'POST',
        ...
        const completeRes = await fetch(
          `/api/upload/session/${encodeURIComponent(uploadId)}/complete`,
          { method: 'POST' }
        )
  ...
  return { uploadFiles, isUploading, uploadProgress, generatePreview }
}
